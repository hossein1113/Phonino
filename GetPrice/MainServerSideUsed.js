...
(async function () {
    ...
    agenda.on("complete", async (job) => {
        try {
            console.log(AgendaJobUrl[job.attrs.data.h1])
            if (job.attrs.data.h1 + 1 < AgendaJobUrl.length){
                agenda.schedule(new Date(Date.now() + 1000), Which[job.attrs.data.h1 + 1], {
                    'h1': job.attrs.data.h1 + 1,
                    'url': encodeURI(AgendaJobUrl[job.attrs.data.h1 + 1])
                    // 'url': 'https://digido.ir/samsung-phone/4018-samsung-galaxy-m22-dual-sim-64gb-ram-4gb-mobile-phone.html#/11-%D8%B1%D9%86%DA%AF-%D9%85%D8%B4%DA%A9%DB%8C/160-%DA%AF%D8%A7%D8%B1%D8%A7%D9%86%D8%AA%DB%8C-18_%D9%85%D8%A7%D9%87%D9%87_%D8%B4%D8%B1%DA%A9%D8%AA%DB%8C_%DA%A9%D8%AF_%D8%B1%D8%AC%DB%8C%D8%B3%D8%AA%D8%B1%DB%8C'
                });
            }
            await job.disable();
            await job.remove();
...
get_all(function (data) {
    get_all_url(function (res) {
        // const shop = ['digikala'];
        const shop = ['digido', 'alldigital', 'digikala'];
            ...
            for (var h1 = 0; h1 < len_os; h1++) {
                if (shop[shop_count] === 'digido') {
                    agenda.define('digido', async (job, done) => {
                        try {
                            await request23(job.attrs.data.url, done, 2, function (res) {
                                ...
                                if (ram_internal.length <= 0) {
                                    ram_internal = $('.st_read_more_box h2');
                                }
                                ...
                                // price = $('table-striped tbody tr td .badge-info'); //use your CSS selector here
                                ...
                                var sort_color = []
                                var sort_garan = []
                                console.log(job.attrs.data.url,"title site")

                                let Pricing = $(".table-striped tbody tr td .badge-info")
                                var table = $(".table-striped tbody");
                                for (var i = 0; i < table.length; i++) {
                                    console.log($(table[i]).find('tr').length , 'table content')
                                    var table = $(".table-striped tbody");
                                    if ($(table[i]).find('tr').length === 1) {
                                        ...
                                        if ($(Pricing[i]).text().replace(' تومان', '').replaceAll(',', '').replaceAll(' ', '') !== 'بهزودی' &&
                                            $(Pricing[i]).text().replaceAll(',', '').replaceAll(' ', '').includes('تومان')) {
                                            sort_price.push(parseInt($(Pricing[i]).text().replace(' تومان', '').replaceAll(',', '').replaceAll(' ', '')));
                                        }
                                    } else {
                                        var link = $('.comb-name')[i];
                                ...
                                internal_ram = $('.product-description-short .st_read_more_box ul li'); //use your CSS selector here
                                var title_site;
                                if (array_title === null) {
                                    array_title = [];
                                }
                                ...
                                console.log(title_site,"title site")
                                if (title_site !== undefined) {
                                    var internal_site;
                                    var ram_site;
                                    $(internal_ram).each(function (i, link) {
                                        console.log($(link).text())
                                        if ($(link).text().includes('حافظه داخلی') || $(link).text().includes('حافظه داخلی') || $(link).text().includes("داخلی") || $(link).text().includes("حافظه")) {
                                            console.log($(link).text() , "in")
                                            array_internal = $(link).text().match('[0-9]+');
                                        ...
                                        if ($(link).text().includes('مقدار رم') || $(link).text().includes('رم')) {
                                            array_ram = $(link).text().match('[0-9]+');
                                    ...
                                    dance:
                                        for (var tot = 0; tot < data.length; tot++) {
                                            ...
                                            for (var b = 0; b < array.length; b++) {
                                                ...
                                                if (title.toLowerCase() === 'samsung galaxy z fold2 5g') {
                                                ...
                                                if (check_internal && check_ram && (title.toLowerCase() === title_site.toLowerCase() || title1.toLowerCase() === title_site.toLowerCase())) {
                                                    if (sort_price.length > 0) {
                                                        var choose_unit;
                                                        if ($(Pricing[0]).text().includes('تومان')) {
                                                            choose_unit = 'تومان';
                                                        } else {
                                                            choose_unit = 'ریال';
                                                        }
                                                        var json_make = '{';
                                                        for (var k = 0; k < sort_price.length; k++) {
                                            ...
                                            if (tot === data.length - 1) {
                                                ...
                                                console.log('Not Exist on database');
                    ...
                    AgendaJobUrl.push(encodeURI(res[0].digido[h1]));
                    Index.push(h1);
                    Which.push("digido");
                    ...
                    AgendaJobUrl.push(encodeURI(res[0].alldigital[h1]));
                    Index.push(h1);
                    Which.push("alldigital")
                    // agenda.schedule(new Date(Date.now() + 1000), 'alldigital', {
                    //     'h1': h1,
                    //     'url': encodeURI(res[0].alldigital[h1])
                    // });
                                ...
                                if (title_site !== undefined || title_site_an_digi !== undefined) {
                                    ...
                                    for (var i = 0; i < haf.length; i++) {
                                        if ($(haf[i]).text().includes('حافظه داخلی')) {
                                            ...
                                            ram = $(haf[i + 1]).text().replace("چهار","4")
                                            ram = ram.replace("هشت","8")
                                            ram = ram.replace("ده","10")
                                            ram = ram.replace("شش","6")
                                            array_ram = ram.match('[1-9]+');
                                            if (array_ram !== null) {
                                                ram_site = array_ram[0];
                                            }
                                            // console.log(ram_site);
                                    ...
                                    dance:
                                        for (var tot = 0; tot < data.length; tot++) {
                                            ...
                                            for (var b = 0; b < array.length; b++) {
                                                ...
                                                // if(title.toLowerCase() === 'samsung galaxy a12' ){
                    ...
                    AgendaJobUrl.push('https://www.digikala.com' + encodeURI(res[0].digikala[h1]));
                    Index.push(h1);
                    Which.push("digikala")
        ...
        agenda.schedule(new Date(Date.now() + 1000), Which[0], {
            'h1': 0,
            'url': AgendaJobUrl[0]
            // 'url': encodeURI(res[0].digido[0])
            // 'url': 'https://digido.ir/samsung-phone/4018-samsung-galaxy-m22-dual-sim-64gb-ram-4gb-mobile-phone.html#/11-%D8%B1%D9%86%DA%AF-%D9%85%D8%B4%DA%A9%DB%8C/160-%DA%AF%D8%A7%D8%B1%D8%A7%D9%86%D8%AA%DB%8C-18_%D9%85%D8%A7%D9%87%D9%87_%D8%B4%D8%B1%DA%A9%D8%AA%DB%8C_%DA%A9%D8%AF_%D8%B1%D8%AC%DB%8C%D8%B3%D8%AA%D8%B1%DB%8C'
        });
    });
});
